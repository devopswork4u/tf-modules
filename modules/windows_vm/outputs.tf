# Outputs for Windows VM module

output "vm_ids" {
  description = "Map of VM IDs"
  value = {
    for key, vm in azurerm_windows_virtual_machine.vm : key => vm.id
  }
}
...
output "vm_names" {
  ...
  value = {
    for key, vm in azurerm_windows_virtual_machine.vm : key => vm.name
  }
}
...
output "vm_details_output" {
  ...
  value = {
    for key, vm in azurerm_windows_virtual_machine.vm : key => {
      id                 = vm.id
      name              = vm.name
